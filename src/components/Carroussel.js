...
class Carroussel extends React.Component {
    ...
    render() {
        ...
        const m = faces.filter(e => e.name === this.state.activeFace)[0]
        const activities = m.core;
        const missions = activities[this.state.mainFace].missions;
        return <React.Fragment>
            ...
            <div className={`menu ${this.state.showingAlert ? 'changeop' : 'nochangeop'} `}>
                ...
                <div className="section" key={m.id}>
                    ...
                    <p></p>
                    <span style={{ fontStyle: "italic" }}> {activities[this.state.mainFace].date}</span>
                    <p></p>
                    <span style={{ fontWeight: "bold" }}>{activities[this.state.mainFace].titre}</span>
                    <p></p>
                    <ul>
                        {missions.map(mission =>
                            (<li style={{ textJustify: "distribute" }}><span className="missionSpan">{mission.name}:</span> {mission.contenu}</li>))}
                    </ul>
                </div>
